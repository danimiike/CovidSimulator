	...
	private class MyJButtonListener implements ActionListener{
		...
		//	{	ver como iremos tratar isso, se for menor que 100 vamos assumir que o resto nao esta vacinado, por exemplo?
				if(popNoShot + popOneShot + popTwoShot + popNatImmunity != 100) {
					JOptionPane.showInternalMessageDialog(contentPane, "The sum of levels of immunity should be 100.");
					
				}
				
				//se passar pelas validacoes vai gerar as pessoas e em seguida o GUI de simulacao
...
public class MainMenu extends JFrame
{
	...
	private int sliderControl, sMin, sMax;
	...
	Font sliderFont = new Font("Times New Roman", Font.PLAIN, 8);
	 ...
	 public MainMenu() {
		...
		panel2 = new JPanel();
		panel2.setLayout(new GridLayout(5,3,0,0));
		panel2.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
		panel2.add(L1);
		panel2.add(s1);
		panel2.add(s1StatusLabel);
		panel2.add(L2);
		panel2.add(s2);
		panel2.add(s2StatusLabel);
		panel2.add(L3);
		panel2.add(s3);
		panel2.add(s3StatusLabel);
		panel2.add(L4);
		panel2.add(s4);
		panel2.add(s4StatusLabel);
		panel2.add(L5);
		panel2.add(s5);
		panel2.add(s5StatusLabel);
...
>>>>>>> 77d4df88c9993e97e2314617668cf0448bb74d56
//end class